...
class ModelEvaluator:
    ...
    def plot_confusion_matrix(self, cm, save_path=None):
        ...
        if save_path:
            ...
            plt.close()
            self.logger.info(f"Confusion matrix plot saved to\n: {save_path}")
        else:
            plt.show()
    ...
    def plot_roc_curve(self, y_test, y_pred_prob, save_path=None):
        ...
        if save_path:
            plt.savefig(save_path, format='pdf')
            plt.close()
            self.logger.info(f"ROC curve plot saved to:\n  {save_path}")
        else:
            plt.show()
    ...
    def plot_precision_recall_curve(self, y_test, y_pred_prob, save_path=None):
        ...
        if save_path:
            plt.savefig(save_path, format='pdf')
            plt.close()
            self.logger.info(f"Precision-Recall curve plot saved to:\n {save_path}")
        else:
            plt.show()
    ...
    def export_metrics_pdf(self, cm, y_test, y_pred_prob, y_pred_class, pdf_filename='metrics_report.pdf'):
        ...
        self.logger.info(f"Metrics exported as PDF:\n {pdf_filename}")
